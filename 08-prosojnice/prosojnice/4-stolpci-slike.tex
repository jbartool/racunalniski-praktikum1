...
\begin{frame}{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
	...
	\centering
		\begin{tikzpicture}
			...
			\coordinate (T') at ({-\tt},{\td});
			\draw[gray, thin] (T') arc[start angle=330, end angle=150, radius=2] node[left] {\scriptsize $m$};
			\pause
			% Definicija točke T'' in risanje loka n z oznako.
			\coordinate (T'') at ({\tt},{\td});
			\draw[gray, thin] (T'') arc[start angle=210, end angle=30, radius=2] node[right] {\scriptsize $n$};
			\pause
			% Definicija in risanje točke C
			\coordinate [label={[blue, above right]:$C$}] (C) at (0,{\t+2*sqrt(3)});
