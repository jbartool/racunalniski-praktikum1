...
\begin{frame}{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
	...
	\centering
		\begin{tikzpicture}
			...
			\coordinate (T') at ({-\td},{\tt});
			\draw[gray, thin] (T') arc[start angle=150, end angle=330, radius=\r] node[left] {\scriptsize $m$};
			}
			\only<+->{%
			% Definicija točke T'' in risanje loka n z oznako.
			\coordinate (T'') at ({\td},{\tt});
			\draw[gray, thin] (T'') arc[start angle=30, end angle=210, radius=\r] node[right] {\scriptsize $n$};
			}
			\only<+->{%
