...
\begin{frame}{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
	...
	\centering
		\begin{tikzpicture}
			\tikzmath{
				\r = 2; % polmer od lokov
				\beta = 30; % Razdalja začetka loka m do premice p oz. razdalja točke T' levo in zgoraj od točke T do premice
				\t = \r * sin(45);% Razdalja od točke T do premice p je tako 2*sin(45°)
				\tt = \r * sin(\beta); % Razdalja točke T' od navpične premice skozi T
				\td = \r * cos(\beta);
			}
			% Definicija točke T
			...
			\only<+->{%
			% Definicija točke T', v kateri se začne lok m in risanje loka m z oznako.
			\coordinate (Tp) at ({-\td},{\tt});
			\draw[gray, thin] (Tp) arc[start angle=150, end angle=330, radius=\r] node[left] {\scriptsize $m$};
			}
			\only<+->{%
			% Definicija točke T'' in risanje loka n z oznako.
			\coordinate (Tpp) at ({\td},{\tt});
			\draw[gray, thin] (Tpp) arc[start angle=30, end angle=210, radius=\r] node[right] {\scriptsize $n$};
			}
			\only<+->{%
			% Definicija in risanje točke C
			\coordinate [label={[blue, above right]:$C$}] (C) at (0,{\t+2*\r*cos(30)});
			\fill[blue] (C) circle (2pt);
			}
			\only<+->{%
			% Risanje premice skozi točki T in C.
			\draw[blue, very thick] (T) -- (C);
			}
		\end{tikzpicture}
	\end{column}
	\end{columns}
