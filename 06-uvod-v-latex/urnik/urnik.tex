...
\subsection[short]{Stroge omejitve} % podpodrazdelek
...
\subsection{Šibke omejitve} % podpodrazdelek
...
kakovost urnika:
\[ k_1 \cdot u_1 + k_2 \cdot u_1 + ... + k_n-1 \cdot u_n-1 + k_n \cdot u_n \]
...
\begin{enumerate}
      ...
      označimo s predmet\(_{i}\).
   
      \item Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
      \( j \)-tem koraku označimo s predavalnica\(_{j}\).
   
      \item Za vse proste termine predavalnice\(_{j} \) izračunamo ceno vstavljanja
...
\subsection{Lokalno vzpenjanje} % podpodrazdelek
...
\subsection{Iskanje s tabuji} % podpodrazdelek
